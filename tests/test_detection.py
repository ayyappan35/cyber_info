...
def test_apply_ceiling_triggers_for_pii_exposure_when_question_does_not_ask_for_pii():
    action, reason = detection.apply_ceiling("rag", "pii-exposure", {"question_requests_personal_info": False})
    assert action == "MITIGATE"
    assert reason


def test_apply_ceiling_no_ceiling_when_question_does_ask_for_pii():
    action, _reason = detection.apply_ceiling("rag", "pii-exposure", {"question_requests_personal_info": True})
    assert action is None
